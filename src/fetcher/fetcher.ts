 ...
 * Supported accounts
 */
type CachedValue = ParsableBattle | ParsableUser;
...
export class AccountFetcher {
    ...
    // YOUR CODE:
    public async getBattle(address: Address, refresh = false): Promise<BattleData | null> {
        return this.get(translateAddress(address), ParsableBattle, refresh);
    }

    public async getUser(address: Address, refresh = false): Promise<UserData | null> {
